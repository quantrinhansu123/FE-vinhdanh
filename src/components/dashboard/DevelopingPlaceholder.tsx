...
export function DevelopingPlaceholder({ title, description, icon = 'construction', id }: DevelopingPlaceholderProps) {
  return (
    <div id={id} className="crm-glass-card rounded-2xl overflow-hidden shadow-2xl border border-crm-outline/30 mt-6 min-h-[400px] flex flex-col items-center justify-center p-12 text-center">
      <motion.div
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        className="w-20 h-20 bg-crm-primary/10 rounded-full flex items-center justify-center mb-6"
      >
        <MIcon name={icon} className="text-4xl text-crm-primary" />
      </motion.div>
      <h3 className="text-2xl font-bold text-crm-on-surface mb-2">{title}</h3>
