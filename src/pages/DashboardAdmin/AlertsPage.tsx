...
export function AlertsPage({
  ...
  return (
    <div id="crm-dash-reconcile" className="crm-glass-card rounded-2xl overflow-hidden border border-crm-outline/30">
      <div className="px-6 lg:px-8 py-5 border-b border-crm-outline/30 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-crm-surface-accent/30">
        <div>
      ...
      <div className="p-6 lg:p-8 space-y-6">
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse min-w-[720px]">
            <thead>
              <tr className="bg-crm-surface-accent/20 border-b border-crm-outline/10">
                <th className="px-4 py-4 text-[10px] font-extrabold text-crm-on-surface-variant uppercase tracking-[0.2em]">Tên Dự Án</th>
                <th className="px-4 py-4 text-[10px] font-extrabold text-crm-on-surface-variant uppercase tracking-[0.2em] text-right">Ngân Sách</th>
                <th className="px-4 py-4 text-[10px] font-extrabold text-crm-on-surface-variant uppercase tracking-[0.2em] text-right">Chi Tiêu Thực</th>
                <th className="px-4 py-4 text-[10px] font-extrabold text-crm-on-surface-variant uppercase tracking-[0.2em] text-right">Chênh Lệch</th>
                <th className="px-4 py-4 text-[10px] font-extrabold text-crm-on-surface-variant uppercase tracking-[0.2em] text-center">% Ads/Doanh Số</th>
                <th className="px-4 py-4 text-[10px] font-extrabold text-crm-on-surface-variant uppercase tracking-[0.2em] text-right">Tình Trạng</th>
              </tr>
            </thead>
              ...
              pagedRows.map((row) => {
                ...
                return (
                  <tr key={`${row.project}-${row.agency}`} className={`${st.rowHover} transition-colors group ${st.warnRow ? 'bg-crm-error/5' : ''}`}>
                    ...
                    <td className="px-4 py-4 text-sm font-medium text-crm-on-surface text-right">{formatVnd(row.budget)}</td>
                    <td className="px-4 py-4 text-sm font-medium text-crm-on-surface text-right">{formatVnd(row.spend)}</td>
                    <td className={`px-4 py-4 text-sm font-extrabold text-right ${row.diffPct > 0 ? 'text-crm-error' : row.diffPct < 0 ? 'text-crm-success' : 'text-crm-on-surface-variant'}`}>
                      {diffStr}
                    </td>
        ...
        <div className="pt-6 border-t border-crm-outline/10 flex flex-col sm:flex-row justify-between items-center gap-4 text-[10px] font-extrabold text-crm-on-surface-variant uppercase tracking-[0.2em]">
