                  ...
                  <table className="w-full text-left border-separate border-spacing-y-2 px-4 min-w-[780px]">
                    ...
                    <tbody className="text-sm">
                      {historyList.length === 0 ? (
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-[var(--ld-on-surface-variant)] leader-dash-label text-xs">
                            ...
                            >
                              ...
                              <td className="px-4 py-4 text-center">
                                <button
                                  ...
                                  className="bg-[var(--ld-surface-container-highest)] p-1.5 rounded text-[var(--ld-primary)] hover:brightness-110 border border-[var(--ld-outline-variant)]/20"
          ...
          <div className="bg-[var(--ld-surface-container-high)] w-full max-w-lg rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-[var(--ld-outline-variant)]/20 overflow-hidden">
            <div className="px-6 py-4 border-b border-[var(--ld-outline-variant)]/10 flex justify-between items-center">
              ...
              <button
                ...
                className="text-[var(--ld-on-surface-variant)] hover:text-[var(--ld-on-surface)]"
              >
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[var(--ld-on-surface-variant)]">Mã yêu cầu:</span>
                  <span className="font-mono text-[var(--ld-primary)] font-bold">#{displayMa(viewRequest.id)}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[var(--ld-on-surface-variant)]">Thời gian duyệt:</span>
                  <span className="text-[var(--ld-on-surface)] font-medium">
                    {formatReqDate(viewRequest.updated_at || viewRequest.ngay_gio_xin)}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[var(--ld-on-surface-variant)]">Trạng thái:</span>
                  <span className={`font-bold ${viewRequest.trang_thai === 'dong_y' ? 'text-[var(--ld-secondary)]' : 'text-[var(--ld-error)]'}`}>
                    {viewRequest.trang_thai === 'dong_y' ? 'Đã duyệt' : 'Từ chối'}
                  </span>
                </div>
                {viewRequest.ly_do_tu_choi && (
                  <div className="p-3 bg-[var(--ld-error-container)]/10 border border-[var(--ld-error)]/20 rounded-lg">
                    <p className="text-xs text-[var(--ld-error)] font-medium">Lý do: {viewRequest.ly_do_tu_choi}</p>
                  </div>
                )}
              </div>

              <div className="space-y-3">
                <p className="text-xs font-bold text-[var(--ld-on-surface-variant)] uppercase tracking-wider">Ảnh minh chứng</p>
                {(() => {
                  const images = [...(viewRequest.chung_tu_urls || []), ...(viewRequest.anh_giai_ngan_urls || [])];
                  if (images.length === 0) return <p className="text-xs text-[var(--ld-on-surface-variant)] italic">Không có ảnh đính kèm.</p>;
                  
                  return (
                    <div className="grid grid-cols-1 gap-4">
                      {images.map((url, idx) => (
                        <div key={idx} className="relative group rounded-xl overflow-hidden border border-[var(--ld-outline-variant)]/20 aspect-video bg-black/20">
            ...
            <div className="p-4 bg-[var(--ld-surface-container-highest)]/30 border-t border-[var(--ld-outline-variant)]/10">
              <button
                type="button"
                onClick={() => setViewRequest(null)}
